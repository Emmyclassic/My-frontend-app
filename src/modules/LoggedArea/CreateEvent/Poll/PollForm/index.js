...
import { pollHandler } from "../../../../../api/resolutionHandler";
...
const ElectionForm = ({
  ...
  const postElection = async (payload) => {
    try {
      const { data } = await pollHandler(payload);
  ...
  const onSubmit = async (data) => {
    if (validateField()) {
      ...
      const transResourceInputCount = resourceInputCount.map((item) => {
        return {
          title: item.title,
          options: item.candidateInputCount,
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      ...
      <button disabled={loading} className="btn-gray">
        {loading ? <Spin size="large" color="#fff" /> : " Create Poll"}
      </button>
      {successResponse && (
        <Alert
...
const ResolutionSelection = ({
  ...
  const placeHolderArr = ["Alex Iwobi", "Mikel  Obi", "Moses Simon"];
  return (
    <>
      <div className={style.form__input_wrap} style={{ position: "relative" }}>
        ...
        <div className={style.form__input_box}>
          ...
          <input
            className={style.form__input}
            type="text"
            placeholder="e.g “Who should be captain of the football team?”"
